...
class GdThumb extends ThumbBase
{
     ...
     * İmaja border çizer
     */
    public function drawBorder ($color, $thickness = 1)
	{

        $img = $this->workingImage;

        // Draw border
        $color_black = ImageColorAllocate($this->workingImage, 0, 0, 0);
        $color = ImageColorAllocate($this->workingImage, 188, 198, 200);
        $color2= ImageColorAllocate($this->workingImage, 227, 232, 235);
        
        $x1 = 0;
        $y1 = 0;
        $x2 = ImageSX($this->workingImage) - 1;
        $y2 = ImageSY($this->workingImage) - 1;

        for($i = 0; $i < $thickness; $i++)
        {
            ImageRectangle($this->workingImage, $x1++, $y1++, $x2--, $y2--, $color);
            ImageColorAllocate($this->workingImage, 255, 255, 255);
            $color = $color2;
        }
